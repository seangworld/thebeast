...
drop trigger if exists set_beast_admin_member_auth_controls_updated_at
...
drop policy if exists "Owners read member authentication controls"
  on public.beast_admin_member_auth_controls;
...
drop policy if exists "Owners read retained member authentication events"
...
create or replace function public.get_beast_admin_member_access_history(
...
create or replace function public.is_current_beast_session_allowed()
returns boolean
language sql
stable
security definer
...
create or replace function public.apply_beast_admin_member_auth_control(
