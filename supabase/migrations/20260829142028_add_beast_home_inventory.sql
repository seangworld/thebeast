...
as $$
  select exists (
    ...
    where profile.id = auth.uid()
      and (
        profile.role = 'admin'
        or profile.birthday <= current_date - interval '18 years'
      )
      and (
        profile.role = 'admin'
        or not exists (
          select 1
          from public.beast_admin_member_module_access access
          where access.member_id = profile.id
            and access.module_id = 'home'
...
create policy "Members manage own home inventories" on public.beast_home_inventories for all to authenticated
  using (owner_id = auth.uid() and public.is_beast_home_member_eligible())
  with check (owner_id = auth.uid() and public.is_beast_home_member_eligible());
...
alter table public.beast_telemetry_events
  drop constraint if exists beast_telemetry_events_event_name_check;
alter table public.beast_telemetry_events
  add constraint beast_telemetry_events_event_name_check check (event_name in (
    'onboarding_completed', 'bill_created', 'debt_created', 'payment_recorded',
    'payoff_plan_viewed', 'education_goal_created', 'education_activity_completed',
    'education_course_created', 'health_workspace_opened', 'health_record_added',
    'appointment_record_added', 'goal_created', 'goal_completed', 'document_uploaded',
    'document_processed', 'document_viewed', 'professional_turn_started',
    'professional_turn_completed', 'professional_turn_failed', 'api_failure',
    'database_command_failed', 'home_inventory_opened', 'home_inventory_started',
    'home_inventory_confirmed', 'home_inventory_exported'
  ));

alter table public.beast_telemetry_events
  drop constraint if exists beast_telemetry_events_module_id_check;
alter table public.beast_telemetry_events
  add constraint beast_telemetry_events_module_id_check check (
    module_id in ('beastos', 'money', 'education', 'health', 'goals', 'documents', 'admin', 'home')
  );

alter function public.get_beast_admin_first_party_telemetry(integer, text)
  rename to get_beast_admin_first_party_telemetry_without_home_inventory;

revoke all on function public.get_beast_admin_first_party_telemetry_without_home_inventory(integer, text)
...
declare
  ...
  home_result jsonb;
begin
  base_result := public.get_beast_admin_first_party_telemetry_without_home_inventory(
    reporting_days, telemetry_environment
  );
  ...
  if actor_count < 5 then
    home_result := jsonb_build_object('status', 'insufficient_data', 'minimumCohort', 5);
  else
    select jsonb_build_object(
      'status', 'available',
      'members', actor_count,
      'opens', count(*) filter (where event_name = 'home_inventory_opened'),
      'starts', count(*) filter (where event_name = 'home_inventory_started'),
      'confirmations', count(*) filter (where event_name = 'home_inventory_confirmed'),
      'exports', count(*) filter (where event_name = 'home_inventory_exported')
    ) into home_result
    from public.beast_telemetry_events
    where environment = telemetry_environment
      and module_id = 'home'
