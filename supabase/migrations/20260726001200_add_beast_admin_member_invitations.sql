...
drop trigger if exists set_beast_households_updated_at
...
drop trigger if exists set_beast_household_memberships_updated_at
...
drop trigger if exists set_beast_admin_member_invitations_updated_at
  on public.beast_admin_member_invitations;
...
drop policy if exists "Household members read their household"
  on public.beast_households;
...
drop policy if exists "Household owners manage their household"
  on public.beast_households;
...
drop policy if exists "Household members read household memberships"
  on public.beast_household_memberships;
...
drop policy if exists "Household owners manage household memberships"
  on public.beast_household_memberships;
...
drop policy if exists "Owners read their member invitations"
  on public.beast_admin_member_invitations;
...
create or replace function public.get_beast_admin_auth_user_id_by_email(
  selected_email text
)
returns uuid
language sql
...
create or replace function public.create_beast_admin_member_invitation(
...
create or replace function public.record_beast_admin_invitation_action(
  selected_actor_id uuid,
...
create or replace function public.accept_beast_admin_member_invitation(
  selected_member_id uuid
)
returns jsonb
language plpgsql
...
create or replace function public.get_beast_admin_member_invitations()
returns jsonb
language plpgsql
stable
security definer
