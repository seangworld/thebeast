-- Preserve distinct dated doses/procedures while blocking same-event duplicates.
...
begin
  ...
  if target_record_id is not null then
    ...
    if exists(select 1 from public.beast_health_records where owner_id=auth.uid() and record_type=target_kind and status <> 'archived' and lower(btrim(title))=lower(btrim(record_title)) and (target_kind not in ('procedure','appointment','vital') or event_date is null or occurred_on is null or occurred_on=event_date)) then
