...
The code-audit route is owner-only and same-origin. It accepts ZIP files up to 12 MB, limits archive entry count and reviewed text size, ignores dependencies and common generated directories, skips unsupported/binary files and links, redacts potential credential values from reports, and never stores or executes the upload. Its output is a screening and handoff aid, not a runtime test, penetration test, live dependency-vulnerability assessment, certification, or guarantee.
