...
The first increment routes only to verified existing factories. KDP uses one master manuscript with Kindle, paperback, and hardcover as output formats. FacelessReels is the current video production engine; BeastMarketing coordinates discovery, scripting, calendar, approvals, publishing, analytics, and optimization. The custom Revision 7 renderer remains preserved but paused. The first client factory accepts an owner-uploaded ZIP, performs a bounded deterministic static review without executing client code or consuming AI credits, and returns a downloadable package containing an executive summary, prioritized technical findings, inventory, delivery notes, and structured audit data. Custom client packages, durable timed runs, cross-factory progress, and batch review remain explicitly disconnected until their BeastFusion execution paths are implemented and verified.
