...
export type BeastAdminCanonicalReadModel = {
  ...
  proposals?: Array<{ id: string; conditionKey: string; title: string; sourceAgent: "observer_agent" | "research_planning_agent"; product: string; detected: string; problem: string; evidence: Array<{ summary: string; source: string; reference: string | null; observedAt: string | null }>; whyItMatters: string; expectedBenefit: string; effort: string; risk: string; scope: string[]; exclusions: string[]; dependencies: string[]; successMeasures: string[]; recommendedPriority: string; recommendation: string; confidence: string; status: string; decisionHistory: Array<{ action: string; rationale: string; detail?: string; decidedAt: string; governanceStatus: string; executionAuthorized: false; executable: false }>; ownerApproved: boolean; executionAuthorized: false; executable: false; createdAt: string; updatedAt: string; source: "beastfusion" }>;
...
export function normalizeBeastAdminCanonicalReadModel(
  ...
  if (
    ...
    (Array.isArray(candidate.proposals) && !requiredStrings(candidate.proposals, ["id", "conditionKey", "title", "sourceAgent", "product", "detected", "whyItMatters", "recommendedPriority", "status", "source"])) ||
...
export function buildBeastAdminCanonicalReadModel(snapshot: BeastFusionStoredSnapshot, options: { configured?: boolean; now?: Date } = {}): BeastAdminCanonicalReadModel {
  ...
  // Roadmap-document indexing is retired from the active operating model.
  // Suppress legacy projection copies while they age out; real blockers,
  // governance errors, missing evidence, and owner decisions remain actionable.
  const legacyRoadmapWarnings = Array.from(new Set(strings(roadmap.warnings)));
  ...
  strings(validation.warnings).filter((detail) => !legacyRoadmapWarnings.includes(detail)).forEach((detail, index) => attention.push({ id: `validation-warning:${index}`, kind: "missing_evidence", detail, source: "beastfusion" }));
  ...
  return {
    ...
    proposals: (projection.proposals ?? []).map(record).map((item) => ({
      id: String(item.id), conditionKey: String(item.conditionKey), title: String(item.title), sourceAgent: item.sourceAgent as "observer_agent" | "research_planning_agent", product: String(item.product), detected: String(item.detected), problem: String(item.problem), evidence: (Array.isArray(item.evidence) ? item.evidence : []).map(record).map((evidence) => ({ summary: String(evidence.summary), source: String(evidence.source), reference: typeof evidence.reference === "string" ? evidence.reference : null, observedAt: typeof evidence.observedAt === "string" ? evidence.observedAt : null })), whyItMatters: String(item.whyItMatters), expectedBenefit: String(item.expectedBenefit), effort: String(item.effort), risk: String(item.risk), scope: strings(item.scope), exclusions: strings(item.exclusions), dependencies: strings(item.dependencies), successMeasures: strings(item.successMeasures), recommendedPriority: String(item.recommendedPriority), recommendation: String(item.recommendation), confidence: String(item.confidence), status: String(item.status), decisionHistory: (Array.isArray(item.decisionHistory) ? item.decisionHistory : []).map(record).map((decision) => ({ action: String(decision.action), rationale: String(decision.rationale), detail: typeof decision.detail === "string" ? decision.detail : undefined, decidedAt: String(decision.decidedAt), governanceStatus: String(decision.governanceStatus), executionAuthorized: false as const, executable: false as const })), ownerApproved: item.ownerApproved === true, executionAuthorized: false as const, executable: false as const, createdAt: String(item.createdAt), updatedAt: String(item.updatedAt), source: "beastfusion" as const,
    })),
    governance: {
