...
export function renderAuditReports(audit: ClientCodeAudit) {
  ...
  const delivery = `# Delivery Notes\n\nThis ZIP is already the complete client-facing Code Risk Scan package. **Do not run it through Client Delivery Package unless combining it with other finished work.**\n\nStart with \`Code-Risk-Scan-Report.pdf\` for the polished client presentation. The HTML report and Markdown files provide alternate working formats. Use the Top Five Actions, CSV tracker, and remediation plan as the work order. Potential credential values and source excerpts are never copied into findings. Review all findings and limitations before delivery.\n`;
