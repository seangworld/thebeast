import { evaluateStandingObservation, evidenceDigest, runWithBoundedRetries, standingProposalSourceId, type ObservationSourceResult } from "../standingObservation";
...
export async function runStandingObservation(ownerId: string, scheduleId: string | null, simulation: Simulation = null) {
  ...
  const [canonical, githubAttempt, vercelAttempt] = await Promise.all([
    loadBeastFusionCanonicalReadModel(),
    simulation ? null : runWithBoundedRetries(readGitHubRepositoryEvidence, (result) => result.provider.status === "error"),
    simulation ? null : runWithBoundedRetries(readVercelDeploymentEvidence, (result) => result.provider.status === "error"),
  ]);
  ...
  if (!canonical.canonical || !canonical.canonical.roadmap.some((item) => item.id === "BF-AGT-011" && item.ownerApproved && item.executionAuthorized)) {
    throw new Error("canonical_assignment_unavailable");
  }
  const sources: ObservationSourceResult[] = simulation === "material" ? [{ source: "controlled_fixture", available: true, changed: true, summary: "Material controlled signal for Preview proof.", confidence: "high", impact: "high", fingerprint: "bf-agt-011-material-v1" }] : simulation === "clean" ? [{ source: "controlled_fixture", available: true, changed: false, summary: "No material change in controlled evidence.", confidence: "high", impact: "none", fingerprint: "bf-agt-011-clean-v1" }] : [
    { source: "beastfusion_canonical_projection", available: true, changed: canonical.canonical.attention.length > 0, summary: canonical.canonical.attention.length ? `${canonical.canonical.attention.length} canonical attention item(s).` : "Canonical governance has no attention items.", confidence: "high", impact: canonical.canonical.attention.some((item) => item.kind === "failure" || item.kind === "blocker") ? "high" : canonical.canonical.attention.length ? "medium" : "none", fingerprint: `${canonical.canonical.projection?.payloadHash || "unknown"}:${canonical.canonical.attention.map((item) => item.id).sort().join(",")}` },
