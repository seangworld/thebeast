...
export async function POST(request: Request) {
  ...
  let conversation: Pick<ConversationRow, "id" | "title" | "message_count">;
  ...
  const context = await loadDirectorContext(supabase, user.id);
  const recommendation = buildDirectorRecommendation({ question, context });
  const createdAt = new Date().toISOString();
  ...
  const { error: updateError } = await supabase
    ...
    .update({
      ...
      summary: {
        ...
        decisions: [],
        unresolvedFollowUps: [recommendation.nextStep],
        updatedAt: createdAt,
      },
