...
export async function POST(request: Request) {
  ...
  if (action === "submit") {
    ...
    const creativeTimingRemediation = job.state === "ready" && quality.ownerQualityReview === "pending" && quality.spokenControlTokenRemediationRenderUsed === true && quality.creativeTimingRemediationRenderUsed !== true;
    if (job.state !== "scripted" && !qualityRemediation && !pronunciationValidation && !controlTokenRemediation && !spokenControlTokenRemediation && !creativeTimingRemediation) return NextResponse.json({ error: "Only a scripted queue item or its bounded pending quality validation can start an internal render." }, { status: 409 });
