...
export function HomeStudioFloorPlan({ project }: { project: HomeStudioProject }) {
  ...
  return <div className="mt-5 rounded-xl border border-[#334155] bg-[#111827] p-4">
    <svg viewBox="0 0 720 500" role="img" aria-label="Dimensioned top-down room-planning outline" className="mx-auto block w-full max-w-3xl">
      ...
      {(project.workspace?.layout || []).map(item => {
        const scale = drawingWidth / geometry.length;
        return <g key={item.id}><rect x={x + item.x * scale} y={y + item.y * scale} width={item.width * scale} height={item.depth * scale} fill={item.kind === "Furniture" ? "#0891b2" : "#a16207"} fillOpacity={0.6} stroke="#67e8f9" /><text x={x + (item.x + item.width / 2) * scale} y={y + (item.y + item.depth / 2) * scale} textAnchor="middle" fill="white" fontSize={11}>{item.label}</text></g>;
      })}
    ...
    <p className="mt-4 text-xs leading-5 text-[#94a3b8]">The room rectangle follows your length-to-width ratio. Wall directions are your labels, not geographic north. Entered furniture and opening markers follow your dimensions; verify fit and clearances on site. This is not a construction drawing.</p>
  </div>;
}
