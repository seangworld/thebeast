...
const roadmapFieldLabels = {
  title: "Plan name",
  destination: "Where you want to go",
  startingPoint: "Where you are now",
  gapSummary: "What remains to do",
} as const;
...
export default function EducationCareerWorkspace({
  ...
  focus?: "about-you" | "education-planning" | "career-planning" | "progress";
}) {
  ...
  async function removeProfile(item: ProfileRow) {
    if (!window.confirm(`Remove “${item.label}” from About You?`)) return;
  ...
  return (
    <div className="grid min-w-0 gap-6" data-education-career-intelligence="BE-201">
      ...
      <section id="profile" className={`${focus && focus !== "about-you" ? "hidden" : ""} rounded-2xl border border-white/10 bg-[#111827] p-4 sm:p-6`} aria-labelledby="education-profile-heading">
        <p className="text-xs font-black uppercase tracking-[0.15em] text-cyan-200">About You</p>
        <h2 id="education-profile-heading" className="mt-2 text-2xl font-black text-white">Your past, today, and what you want next</h2>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">Share only what feels useful. Your Guidance Counselor uses it to give advice that fits your real life.</p>
        ...
        <form className="mt-5 grid gap-3 rounded-xl border border-cyan-300/15 bg-cyan-300/[0.04] p-4 md:grid-cols-2" onSubmit={saveProfile}>
          <h3 className="font-black text-white md:col-span-2">{editingProfileId ? "Update this" : "Add something about you"}</h3>
          <label className="grid gap-1 text-sm text-slate-300">Which part of your story?<select className="beast-input" value={profileDraft.phase} onChange={(event) => setProfileDraft((current) => ({ ...current, phase: event.target.value as EducationCareerProfilePhase }))}><option value="past">Past</option><option value="present">Present</option><option value="goal">Goal</option></select></label>
          <label className="grid gap-1 text-sm text-slate-300">What is this about?<select className="beast-input" value={profileDraft.category} onChange={(event) => setProfileDraft((current) => ({ ...current, category: event.target.value }))}>{profileCategories.map((category) => <option key={category} value={category}>{titleCase(category)}</option>)}</select></label>
          <label className="grid gap-1 text-sm text-slate-300">Short title<input className="beast-input" required maxLength={200} value={profileDraft.label} onChange={(event) => setProfileDraft((current) => ({ ...current, label: event.target.value }))} /></label>
          ...
          <div className="flex flex-wrap gap-2 md:col-span-2"><button className="beast-button-primary" type="submit">{editingProfileId ? "Save change" : "Save this"}</button>{editingProfileId ? <button className="beast-button-secondary" type="button" onClick={() => { setEditingProfileId(""); setProfileDraft({ phase: "present", category: "role", label: "", value: "", occurredOn: "" }); }}>Cancel</button> : null}</div>
        </form>
      </section>

      <section id="paths" className={`${focus && focus !== "career-planning" ? "hidden" : ""} rounded-2xl border border-white/10 bg-[#111827] p-4 sm:p-6`} aria-labelledby="paths-heading">
        <p className="text-xs font-black uppercase tracking-[0.15em] text-violet-200">Career Options</p>
        <h2 id="paths-heading" className="mt-2 text-2xl font-black text-white">Compare careers that could fit you</h2>
        <p className="mt-2 text-sm leading-6 text-slate-300">We will show what is known, what still needs checking, and why each option may or may not fit.</p>
        ...
        </article>)}</div> : <p className="mt-5 rounded-xl border border-white/10 bg-black/15 p-4 text-sm text-slate-400">No careers are being compared yet. Add one career you want to understand better.</p>}
        <form className="mt-5 grid gap-3 rounded-xl border border-violet-300/15 bg-violet-300/[0.04] p-4 md:grid-cols-2" onSubmit={savePath}>
          <h3 className="font-black text-white md:col-span-2">Add a career to compare</h3>
          <label className="grid gap-1 text-sm text-slate-300">Career or option name<input className="beast-input" required value={pathDraft.title} onChange={(event) => setPathDraft((current) => ({ ...current, title: event.target.value }))} /></label>
          <label className="grid gap-1 text-sm text-slate-300">Type of work<select className="beast-input" value={pathDraft.pathType} onChange={(event) => setPathDraft((current) => ({ ...current, pathType: event.target.value }))}>{["promotion", "federal", "contractor", "private_sector", "college", "graduate_degree", "certification", "trade", "licensure", "entrepreneurship", "alternative_occupation", "other"].map((value) => <option key={value} value={value}>{titleCase(value)}</option>)}</select></label>
          <label className="grid gap-1 text-sm text-slate-300 md:col-span-2">Education goal<select className="beast-input" value={pathDraft.goalId} onChange={(event) => setPathDraft((current) => ({ ...current, goalId: event.target.value }))}><option value="">No goal selected</option>{goals.map((goal) => <option key={goal.id} value={goal.id}>{goal.title} · {goal.status}</option>)}</select></label>
          ...
          <label className="grid gap-1 text-sm text-slate-300">What still needs checking<textarea className="beast-input" value={pathDraft.risk} onChange={(event) => setPathDraft((current) => ({ ...current, risk: event.target.value }))} /></label>
          <button className="beast-button-primary w-fit md:col-span-2" type="submit">Save career option</button>
        </form>
      </section>

      <section id="roadmap" className={`${focus && focus !== "education-planning" ? "hidden" : ""} rounded-2xl border border-white/10 bg-[#111827] p-4 sm:p-6`} aria-labelledby="roadmap-heading">
        <p className="text-xs font-black uppercase tracking-[0.15em] text-indigo-200">Your Education Plan</p>
        <h2 id="roadmap-heading" className="mt-2 text-2xl font-black text-white">Steps you choose and can change</h2>
        <p className="mt-2 text-sm leading-6 text-slate-300">Your counselor can suggest changes, but you decide what becomes part of your plan. You can pause, resume, reorder, or archive it.</p>
        <div className="mt-5 grid gap-4">{roadmaps.map((roadmap) => <article key={roadmap.id} className="rounded-xl border border-white/10 bg-black/15 p-4">
          ...
          <dl className="mt-4 grid gap-3 md:grid-cols-3"><div><dt className="text-xs font-bold uppercase text-slate-500">Destination</dt><dd className="mt-1 text-sm text-slate-200">{roadmap.destination || "Not defined"}</dd></div><div><dt className="text-xs font-bold uppercase text-slate-500">Starting point</dt><dd className="mt-1 text-sm text-slate-200">{roadmap.starting_point || "Not defined"}</dd></div><div><dt className="text-xs font-bold uppercase text-slate-500">What remains to do</dt><dd className="mt-1 text-sm text-slate-200">{roadmap.gap_summary || "Not assessed"}</dd></div></dl>
        ...
        {!roadmaps.length ? <p className="mt-5 rounded-xl border border-white/10 bg-black/15 p-4 text-sm text-slate-400">No education plan exists yet. Start with a simple draft, then review it before making it active.</p> : null}
        <div className="mt-5 grid gap-4 lg:grid-cols-2"><form className="grid gap-3 rounded-xl border border-indigo-300/15 bg-indigo-300/[0.04] p-4" onSubmit={saveRoadmap}><h3 className="font-black text-white">Start an education plan</h3><label className="grid gap-1 text-sm text-slate-300">Education goal<select className="beast-input" value={roadmapDraft.goalId} onChange={(event) => setRoadmapDraft((current) => ({ ...current, goalId: event.target.value }))}><option value="">No goal selected</option>{goals.map((goal) => <option key={goal.id} value={goal.id}>{goal.title} · {goal.status}</option>)}</select></label>{(["title", "destination", "startingPoint", "gapSummary"] as const).map((field) => <label key={field} className="grid gap-1 text-sm text-slate-300">{roadmapFieldLabels[field]}<textarea className="beast-input" required={field === "title"} value={roadmapDraft[field]} onChange={(event) => setRoadmapDraft((current) => ({ ...current, [field]: event.target.value }))} /></label>)}<button className="beast-button-primary w-fit" type="submit">Save plan</button></form><form className="grid content-start gap-3 rounded-xl border border-indigo-300/15 bg-indigo-300/[0.04] p-4" onSubmit={saveStep}><h3 className="font-black text-white">Add a step</h3><label className="grid gap-1 text-sm text-slate-300">Education plan<select className="beast-input" required value={stepDraft.roadmapId} onChange={(event) => setStepDraft((current) => ({ ...current, roadmapId: event.target.value }))}><option value="">Select a plan</option>{roadmaps.map((roadmap) => <option key={roadmap.id} value={roadmap.id}>{roadmap.title}</option>)}</select></label><label className="grid gap-1 text-sm text-slate-300">Step<input className="beast-input" required value={stepDraft.title} onChange={(event) => setStepDraft((current) => ({ ...current, title: event.target.value }))} /></label><label className="grid gap-1 text-sm text-slate-300">Target date<input className="beast-input" type="date" value={stepDraft.targetDate} onChange={(event) => setStepDraft((current) => ({ ...current, targetDate: event.target.value }))} /></label><button className="beast-button-primary w-fit" type="submit">Add step</button></form></div>
      </section>

      <section id="research" className={`${focus ? "hidden" : ""} rounded-2xl border border-white/10 bg-[#111827] p-4 sm:p-6`} aria-labelledby="research-heading">
      ...
      <section id="documents" className={`${focus ? "hidden" : ""} rounded-2xl border border-white/10 bg-[#111827] p-4 sm:p-6`} aria-labelledby="documents-heading"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.15em] text-cyan-200">Documents &amp; Evidence</p><h2 id="documents-heading" className="mt-2 text-2xl font-black text-white">Review before anything becomes authoritative</h2></div><Link className="beast-button-secondary" href="/dashboard/education/documents">Open Education documents</Link></div>
      ...
      <section id="outcomes" className={`${focus && focus !== "progress" ? "hidden" : ""} rounded-2xl border border-white/10 bg-[#111827] p-4 sm:p-6`} aria-labelledby="outcomes-heading"><p className="text-xs font-black uppercase tracking-[0.15em] text-emerald-200">Your Progress</p><h2 id="outcomes-heading" className="mt-2 text-2xl font-black text-white">Important choices and achievements</h2>
        ...
        <form className="mt-5 grid gap-3 rounded-xl border border-emerald-300/15 bg-emerald-300/[0.04] p-4 md:grid-cols-2" onSubmit={saveOutcome}><label className="grid gap-1 text-sm text-slate-300">What kind of update?<select className="beast-input" value={outcomeDraft.eventType} onChange={(event) => setOutcomeDraft((current) => ({ ...current, eventType: event.target.value }))}>{["path_considered", "recommendation", "decision", "application", "enrollment", "credential_attempt", "credential_earned", "interview", "offer", "promotion", "rejection", "deferral", "direction_change", "reflection"].map((value) => <option key={value} value={value}>{titleCase(value)}</option>)}</select></label><label className="grid gap-1 text-sm text-slate-300">Title<input className="beast-input" required value={outcomeDraft.title} onChange={(event) => setOutcomeDraft((current) => ({ ...current, title: event.target.value }))} /></label><label className="grid gap-1 text-sm text-slate-300 md:col-span-2">What happened and what did you learn?<textarea className="beast-input min-h-20" value={outcomeDraft.detail} onChange={(event) => setOutcomeDraft((current) => ({ ...current, detail: event.target.value }))} /></label><button className="beast-button-primary w-fit md:col-span-2" type="submit">Save update</button></form>
      </section>

      <section className={`${focus ? "hidden" : ""} grid gap-3 sm:grid-cols-3`} aria-label="More planning tools"><Link className="rounded-xl border border-white/10 bg-white/[0.035] p-4 font-black text-white" href="/dashboard/education/goals">Education Goals <span aria-hidden="true">→</span></Link><Link className="rounded-xl border border-white/10 bg-white/[0.035] p-4 font-black text-white" href="/dashboard/education/documents">Education Documents <span aria-hidden="true">→</span></Link><Link className="rounded-xl border border-white/10 bg-white/[0.035] p-4 font-black text-white" href="/dashboard/education/guidance-counselor">Discuss with Guidance Counselor <span aria-hidden="true">→</span></Link></section>
    </div>
  );
}
