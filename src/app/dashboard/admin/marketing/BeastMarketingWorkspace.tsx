  ...
  return <div className="space-y-6">
    ...
    {selectedCampaign ? <>
      ...
      <SectionHeader eyebrow="BeastMarketing Specialist" title="Continue, modify, or stop" description="The recommendation is deterministic and advisory. It cannot approve, publish, schedule, or spend." action={<button type="button" className="beast-button" disabled={Boolean(busy)} onClick={() => void send({ kind: "recommendation", campaignId: selectedCampaign.id }, "A new evidence-backed recommendation was recorded.")}>Generate recommendation</button>} />
        {selectedCampaign.sourceFacts.some((fact) => fact.label.startsWith("Search Console sampled query: ")) ? <div className="mt-4"><button type="button" className="beast-button" disabled={Boolean(busy) || loading} onClick={() => void send({ kind: "search_assessment", campaignId: selectedCampaign.id }, "A dated search assessment was saved. The original campaign baseline is preserved.")}>{busy === "search_assessment" ? "Refreshing search evidence…" : "Refresh search evidence"}</button><p className="mt-2 text-xs text-slate-400">Compare the latest finalized 30 days with the prior 30 days for this campaign’s exact page and query. Saved assessments below are historical; their dates identify the evidence available at the time.</p></div> : null}
