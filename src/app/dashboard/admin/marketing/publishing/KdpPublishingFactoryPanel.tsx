...
type Publication = { id: string; title: string; audience: string; topic: string; formats: string[]; state: string; opportunity_score: number | null; updated_at: string };
...
export function KdpPublishingFactoryPanel() {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
      ...
      <div className="mt-4 space-y-3">{publications.map((item) => <article key={item.id} className="rounded-xl border border-white/10 bg-slate-950/50 p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-black text-white">{item.title}</h3><p className="mt-1 text-sm text-slate-300">{item.topic}</p></div><span className="rounded-full border border-amber-300/20 px-3 py-1 text-xs font-black text-amber-100">Score {item.opportunity_score ?? "Unavailable"}</span></div><p className="mt-3 text-xs text-slate-400">{label(item.state)} · {item.formats.map(label).join(" · ")} · {item.audience}</p></article>)}</div>
    </section>
  </div>;
}
