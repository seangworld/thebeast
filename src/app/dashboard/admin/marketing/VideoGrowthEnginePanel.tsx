...
function OwnerProductionWorkflow({ series, jobs, selectedId, onSelectSeries, busy, pauseAllPublishing, authorities, onGenerate, onSend }: { series: Series[]; jobs: Job[]; selectedId: string; onSelectSeries: (id: string) => void; busy: boolean; pauseAllPublishing: boolean; authorities: Snapshot["authorities"]; onGenerate: (mode: "test" | "batch", topicFamily: string, count: number) => Promise<void>; onSend: Send }) {
  ...
  const grouped = useMemo(() => {
    const ordered = [...jobs].sort((left, right) => {
      const leftActive = left.provenance?.visualTemplate === "news-acceptance2-v1" ? 1 : 0;
      const rightActive = right.provenance?.visualTemplate === "news-acceptance2-v1" ? 1 : 0;
...
function OwnerWorkflowCard({ job, seriesName, busy, publishingLocked, onSend }: { job: Job; seriesName: string; busy: boolean; publishingLocked: boolean; onSend: Send }) {
  ...
  const acceptance2 = job.provenance?.visualTemplate === "news-acceptance2-v1";
  ...
  return <article className="rounded-xl border border-white/10 bg-slate-950/40 p-4">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-black text-white">{job.topic.title || "Untitled candidate"}</p><p className="mt-1 text-xs text-slate-400">{seriesName}{job.provenance?.candidateCount ? ` · Candidate ${job.provenance.candidateIndex || 1} of ${job.provenance.candidateCount}` : ""}</p>{acceptance2 ? <p className="mt-2 text-xs font-black uppercase tracking-wider text-green-200">CURRENT ACTIVE CANDIDATE · Acceptance Test #2</p> : historicalNewsTest ? <p className="mt-2 text-xs font-black uppercase tracking-wider text-slate-500">Historical News test candidate</p> : null}<p className="mt-1 text-xs text-slate-500">{runtimeSeconds ? `${runtimeSeconds}s` : "Runtime pending"}{visualBeatCount ? ` · ${visualBeatCount} visual beats` : ""}{typeof qualityScore === "number" ? ` · quality ${qualityScore}/100` : ""} · revision {job.revision || 1}{created ? ` · created ${new Date(created).toLocaleString()}` : ""}</p></div><span className="rounded-full border border-amber-300/25 bg-amber-300/[0.07] px-3 py-1 text-xs font-black text-amber-100">{ownerWorkflowStatus(job)}</span></div>
