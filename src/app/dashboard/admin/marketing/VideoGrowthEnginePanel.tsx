...
type Job = { id: string; series_id: string; state: VideoJobState; revision?: number; created_at?: string; topic: JobTopic; script?: { estimatedSeconds?: number; warnings?: string[] }; production?: { manifest?: { checksum: string; runtimeMs: number; width: number; height: number; scenes: unknown[]; planState: string; blockers: string[]; timingEvidenceRequired?: boolean; visualPlan?: { beats?: unknown[] } }; providerState?: string; externalActionPerformed?: boolean; renderAuthorizationRequired?: boolean; timingEvidenceRequired?: boolean; syncVerificationRequired?: boolean; timingPreparation?: { state?: string; narrationAssetId?: string; narrationAudioUrl?: string | null; transcriptionSourceId?: string; transcriptionSrtUrl?: string | null; actualNarrationDurationMs?: number | null; providerStatus?: string; error?: string; finalRenderSubmitted?: boolean; estimatedCredits?: { speechCredits?: number; transcriptionCredits?: number | null; total?: number }; actualCredits?: number | null }; technicalRetry?: { authorizedByOwner?: boolean; maximumAttempts?: number; attemptsConsumed?: number }; estimatedCredits?: { estimatedTotal: number; basis: string } }; quality?: { warnings?: string[]; renderReady?: boolean; ownerQualityReview?: string; ownerWorkflowDecision?: string; ownerApprovalSource?: string; ownerQualityGrade?: string; technicalResult?: string; creativeResult?: string; voiceReview?: string; pacingReview?: string; visualFramingReview?: string; motionTreatmentReview?: string; qualityScore?: number; runtimeSeconds?: number; visualBeatCount?: number; timingEvidenceBound?: boolean; syncVerificationRequired?: boolean; syncMethod?: string; autoApprovalBlockers?: string[]; remediationRenderUsed?: boolean; narrationNormalizationRenderUsed?: boolean; controlTokenRemediationRenderUsed?: boolean }; provenance?: { generatedBy?: string; generationMode?: string; generatedAt?: string; waitingForOwnerApproval?: boolean; candidateIndex?: number; candidateCount?: number; visualTemplate?: string; candidateLabel?: string; revisionLabel?: string; acceptanceTest?: number; activeCandidate?: boolean; superseded?: boolean; visualPresentationRevision?: boolean; revision6SyncRuntime?: boolean; revision7FinalSync?: boolean; timingEvidenceRequired?: boolean; timingEvidenceBound?: boolean; syncVerificationRequired?: boolean; syncMethod?: string; technicalRetryAuthorized?: boolean; cadencePlan?: { plannedFor?: string } }; latestAttempt?: { attemptNumber?: number; status?: string; errorCategory?: string | null; providerRequestId?: string | null; providerHttpStatus?: number | null; providerErrorCode?: string | null; providerErrorMessage?: string | null; providerValidationPath?: string | null; createdAt?: string; completedAt?: string | null } | null; technicalRecovery?: { available?: boolean; attemptNumber?: number; providerHttpStatus?: number | null }; last_error?: string | null; updated_at: string };
type Snapshot = {
...
export function VideoGrowthEnginePanel() {
  ...
  useEffect(() => {
    ...
    refresh();
    window.addEventListener("beast-video-updated", refresh);
    return () => window.removeEventListener("beast-video-updated", refresh);
  }, []);
  ...
  async function send(method: "POST" | "PATCH", payload: Record<string, unknown>, success: string) {
    ...
    try {
      ...
      setMessage(success); window.dispatchEvent(new Event("beast-video-updated"));
  ...
  return <div className="space-y-6">
    <DashboardCard accent="admin">
      ...
      <div className={`mt-5 rounded-2xl border p-5 ${data.controls.pause_all_publishing ? "border-red-300/40 bg-red-400/[0.08]" : "border-amber-300/30 bg-amber-300/[0.05]"}`}>
        <div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-black uppercase tracking-widest text-red-100">Global publishing interlock</p><p className="mt-1 text-sm text-slate-300">{data.controls.pause_all_publishing ? "PAUSE ALL PUBLISHING is active." : "Global pause is released. Private uploads still require a reviewed video and your confirmation."}</p></div><button className="beast-button" disabled={Boolean(busy)} onClick={() => void send("PATCH", { kind: "controls", pauseAllPublishing: !data.controls.pause_all_publishing }, data.controls.pause_all_publishing ? "Global pause released. Public and automatic publishing remain disabled." : "All publishing paused.")}>{data.controls.pause_all_publishing ? "Release global pause" : "PAUSE ALL PUBLISHING"}</button></div>
      </div>
      ...
      <p className="mt-4 text-sm text-amber-100">Public and automatic publishing remain disabled. Use Direct YouTube above for an explicit private upload after reviewing a finished video.</p>
...
function OwnerProductionWorkflow({ series, jobs, selectedId, onSelectSeries, busy, pauseAllPublishing, authorities, onGenerate, onSend }: { series: Series[]; jobs: Job[]; selectedId: string; onSelectSeries: (id: string) => void; busy: boolean; pauseAllPublishing: boolean; authorities: Snapshot["authorities"]; onGenerate: (mode: "test" | "batch", topicFamily: string, count: number) => Promise<void>; onSend: Send }) {
  ...
  const grouped = useMemo(() => {
    const ordered = [...jobs].sort((left, right) => {
      const leftActive = left.provenance?.activeCandidate === true && /^news-acceptance2-v\d+$/i.test(left.provenance?.visualTemplate || "") ? 1 : 0;
      const rightActive = right.provenance?.activeCandidate === true && /^news-acceptance2-v\d+$/i.test(right.provenance?.visualTemplate || "") ? 1 : 0;
      return rightActive - leftActive || Date.parse(right.updated_at || right.created_at || "") - Date.parse(left.updated_at || left.created_at || "");
    });
    return Object.fromEntries(workflowSections.map((section) => [section.id, ordered.filter((job) => ownerWorkflowGroup(job) === section.id)])) as Record<OwnerWorkflowGroup, Job[]>;
  }, [jobs]);
  ...
  return <DashboardCard accent="admin">
    <SectionHeader eyebrow="BMKT-007 · Owner production workflow" title="Generate, review, approve, and track video candidates" description="Series are reusable content strategies. Prepare one test candidate or a bounded batch from a multi-word topic family, then follow the next step on each card to evaluate, script, render, and review." />
    <div className="mt-5 rounded-2xl border border-amber-300/25 bg-amber-300/[0.05] p-5">
      <div className="grid gap-4 lg:grid-cols-[1fr_1.4fr_9rem_auto_auto]">
        ...
        <button className="beast-button self-end" disabled={busy || !selectedId || !topicFamily.trim()} onClick={() => void onGenerate("test", topicFamily, 1)}>Prepare Video Idea</button>
        <button className="beast-button self-end" disabled={busy || !selectedId || !topicFamily.trim()} onClick={() => void onGenerate("batch", topicFamily, count)}>Prepare Idea Batch</button>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-xs font-black"><span className={`rounded-full border px-3 py-1 ${pauseAllPublishing ? "border-red-300/30 text-red-100" : "border-white/10 text-slate-300"}`}>{pauseAllPublishing ? "PAUSE ALL PUBLISHING active" : "Global pause released"}</span><span className="rounded-full border border-white/10 px-3 py-1 text-slate-300">{selectedSeries?.settings.approvalMode === "automatic" ? `Auto-Approve configured · first ${selectedSeries.settings.manualApprovalFirstN} manual` : "Owner Approval required"}</span><span className="rounded-full border border-white/10 px-3 py-1 text-slate-300">Automatic publishing authority {authorities.automaticPublishing === "enabled" ? "enabled" : "locked"}</span><span className="rounded-full border border-white/10 px-3 py-1 text-slate-300">YouTube: {label(authorities.youtube)}</span></div>
    </div>
...
function OwnerWorkflowCard({ job, seriesName, busy, publishingLocked, onSend }: { job: Job; seriesName: string; busy: boolean; publishingLocked: boolean; onSend: Send }) {
  ...
  const acceptance2 = job.provenance?.acceptanceTest === 2 || /^news-acceptance2-v\d+$/i.test(job.provenance?.visualTemplate || "");
  const activeAcceptance2 = acceptance2 && job.provenance?.activeCandidate !== false && !job.provenance?.superseded;
  const reviewRecorded = job.quality?.ownerQualityGrade === "B" && job.quality?.technicalResult === "PASS" && job.quality?.creativeResult === "REVISION REQUIRED";
  const visualPresentationReady = activeAcceptance2 && reviewRecorded && job.quality?.visualFramingReview === "NEEDS REVISION" && job.quality?.motionTreatmentReview === "NEEDS REVISION" && job.provenance?.visualPresentationRevision !== true;
  const revision6Ready = activeAcceptance2 && job.revision === 5 && job.provenance?.visualPresentationRevision === true && job.quality?.renderReady === true;
  const revision7Ready = activeAcceptance2 && job.revision === 6 && job.provenance?.revision6SyncRuntime === true;
  const revision7TimingReady = activeAcceptance2 && job.revision === 7 && job.provenance?.revision7FinalSync === true && job.provenance?.timingEvidenceBound !== true;
  const timingState = job.production?.timingPreparation?.state;
  const historicalNewsTest = job.provenance?.visualTemplate === "news-visual-test-v1";
  const runtimeSeconds = job.quality?.runtimeSeconds ?? (job.production?.manifest?.runtimeMs ? job.production.manifest.runtimeMs / 1000 : job.script?.estimatedSeconds);
  const visualBeatCount = job.quality?.visualBeatCount ?? job.production?.manifest?.visualPlan?.beats?.length;
  const qualityScore = job.quality?.qualityScore;
  const created = job.created_at || job.provenance?.generatedAt;
  ...
  return <article className="rounded-xl border border-white/10 bg-slate-950/40 p-4">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-black text-white">{job.topic.title || "Untitled candidate"}</p><p className="mt-1 text-xs text-slate-400">{seriesName}{job.provenance?.candidateCount ? ` · Candidate ${job.provenance.candidateIndex || 1} of ${job.provenance.candidateCount}` : ""}</p>{activeAcceptance2 ? <p className="mt-2 text-xs font-black uppercase tracking-wider text-green-200">CURRENT ACTIVE CANDIDATE · Acceptance Test #2{job.provenance?.revisionLabel ? ` — ${job.provenance.revisionLabel.replace(/^Acceptance Test #2\s*—\s*/i, "")}` : ""}</p> : job.provenance?.superseded ? <p className="mt-2 text-xs font-black uppercase tracking-wider text-slate-500">SUPERSEDED · retained for audit history</p> : historicalNewsTest ? <p className="mt-2 text-xs font-black uppercase tracking-wider text-slate-500">Historical News test candidate</p> : null}<p className="mt-1 text-xs text-slate-500">{runtimeSeconds ? `${runtimeSeconds}s` : "Runtime pending"}{visualBeatCount ? ` · ${visualBeatCount} visual beats` : ""}{typeof qualityScore === "number" ? ` · quality ${qualityScore}/100` : ""} · revision {job.revision || 1}{created ? ` · created ${new Date(created).toLocaleString()}` : ""}</p>{reviewRecorded ? <p className="mt-1 text-xs font-bold text-amber-100">Owner review: B · technical PASS · creative revision required{job.quality?.visualFramingReview ? ` · framing ${job.quality.visualFramingReview.toLowerCase()}` : ""}{job.quality?.motionTreatmentReview ? ` · motion ${job.quality.motionTreatmentReview.toLowerCase()}` : ""}</p> : null}{job.latestAttempt?.status === "failed" ? <p className="mt-1 text-xs font-bold text-red-200">Shotstack attempt failed · {job.latestAttempt.errorCategory || "provider"}{job.latestAttempt.providerHttpStatus ? ` · HTTP ${job.latestAttempt.providerHttpStatus}` : ""}{job.latestAttempt.providerErrorCode ? ` · ${job.latestAttempt.providerErrorCode}` : ""}{job.latestAttempt.providerValidationPath ? ` · ${job.latestAttempt.providerValidationPath}` : ""}</p> : null}</div><span className="rounded-full border border-amber-300/25 bg-amber-300/[0.07] px-3 py-1 text-xs font-black text-amber-100">{ownerWorkflowStatus(job)}</span></div>
    {job.provenance?.generatedBy === "BeastMarketing" ? <p className="mt-3 text-sm font-bold text-green-200">{finished ? "The rendered video is available below for review." : "Idea prepared. Follow the production steps below; no finished video exists yet."}</p> : null}
    ...
    {!held && ["idea", "modify"].includes(job.state) ? <OpportunityEvaluation job={job} busy={busy} onSend={onSend} /> : null}
    {!held && ["selected", "modify"].includes(job.state) ? <ScriptBuilder job={job} busy={busy} onSend={onSend} /> : null}
    {!held && ["scripted", "generating", "ready"].includes(job.state) ? <ProductionPlanner job={job} busy={busy} onSend={onSend} /> : null}
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      {finished && job.topic.title === "SEANGWORLD News" ? <><button className="beast-button" disabled={busy} onClick={() => void onSend("POST", { kind: "prepare_news_visual_test", id: job.id }, "Image-backed test prepared without spending credits. The original video is unchanged.")}>Prepare image-backed test</button><button className="beast-button" disabled={busy} onClick={() => void onSend("POST", { kind: "prepare_news_acceptance2", id: job.id }, "Acceptance Test #2 candidate prepared with a 100/100 preflight. No Shotstack credits were used.")}>Prepare Acceptance Test #2</button></> : null}
      {job.technicalRecovery?.available ? <button className="beast-button" disabled={busy} onClick={() => void onSend("POST", { kind: "create_corrected_revision", id: job.id }, "Corrected revision created. One Owner-authorized Shotstack attempt is available; no render was submitted.")}>Create corrected revision · 1 render authorized</button> : null}
      {finished && acceptance2 && job.revision === 4 && !reviewRecorded ? <button className="beast-button" disabled={busy} onClick={() => void onSend("PATCH", { kind: "owner_review", id: job.id, decision: "needs_changes", grade: "B", technicalResult: "PASS", creativeResult: "REVISION REQUIRED", voiceReview: "ACCEPTED", pacingReview: "ACCEPTED", visualFramingReview: "NEEDS REVISION", motionTreatmentReview: "NEEDS REVISION" }, "Revision 4 Owner Review recorded: B, with visual framing and motion revision required.")}>Record Revision 4 Owner Review</button> : null}
      {visualPresentationReady ? <button className="beast-button" disabled={busy} onClick={() => void onSend("POST", { kind: "create_visual_presentation_revision", id: job.id }, "Revision 5 prepared with full-frame static screenshot presentation. No Shotstack credits were used.")}>Create Revision 5 · static full-frame presentation</button> : null}
      {revision6Ready ? <button className="beast-button" disabled={busy} onClick={() => void onSend("POST", { kind: "create_revision6_sync_runtime", id: job.id }, "Revision 6 prepared with a 61.5-second synchronized presentation. No Shotstack credits were used.")}>Create Revision 6 · 60s synchronized presentation</button> : null}
      {revision7Ready ? <button className="beast-button" disabled={busy} onClick={() => void onSend("POST", { kind: "create_revision7_final_sync", id: job.id }, "Revision 7 prepared with clean fades and timing evidence required before rendering. No Shotstack credits were used.")}>Create Revision 7 · final sync + clean fades</button> : null}
      {revision7TimingReady && !timingState ? <button className="beast-button" disabled={busy} onClick={() => void onSend("POST", { kind: "prepare_revision7_timing", id: job.id }, "Actual narration timing preparation started. Shotstack will not render or publish anything.")}>Prepare actual narration timing</button> : null}
      {revision7TimingReady && timingState && timingState !== "ready" && timingState !== "failed" ? <button className="beast-button" disabled={busy} onClick={() => void onSend("POST", { kind: "check_revision7_timing", id: job.id }, "Narration timing status checked. No final composition render was submitted.")}>Check narration timing status</button> : null}
      {job.revision === 7 && job.provenance?.timingEvidenceRequired === true && job.provenance?.timingEvidenceBound !== true ? <span className="self-center text-xs font-bold text-amber-100">Actual narration timing evidence is required before render. Preparation uses Shotstack Create and Ingest only; final composition remains blocked.</span> : null}
      {finished && decision !== "approved" ? <button className="beast-button" disabled={busy} onClick={() => void onSend("PATCH", { kind: "owner_review", id: job.id, decision: "approved" }, "Video approved. The Direct YouTube render list will refresh for private upload.")}>Approve reviewed video</button> : null}
      ...
      {(decision === "held" && ["idea", "selected", "scripted", "generating", "ready"].includes(job.state)) || (decision === "approved" && job.state === "ready") ? <button className="min-h-10 rounded-lg border border-white/15 px-3 text-sm font-black text-white disabled:opacity-50" disabled={busy} onClick={() => void onSend("PATCH", { kind: "owner_review", id: job.id, decision: "pending" }, "Candidate resumed at its current production step.")}>{finished ? "Return to Needs Review" : "Resume preparation"}</button> : null}
      ...
      {decision === "approved" && publishingLocked ? <span className="self-center text-xs font-bold text-slate-500">Public scheduling is disabled. Use Direct YouTube above for private upload.</span> : null}
    </div>
  </article>;
}
...
function OpportunityEvaluation({ job, busy, onSend }: { job: Job; busy: boolean; onSend: Send }) {
  ...
  return <details open className="mt-4 rounded-xl border border-white/10 p-4"><summary className="cursor-pointer text-sm font-black text-white">Evaluate Product Truth and funnel fit</summary><p className="mt-2 text-sm text-slate-400">{job.topic.rationale?.join(" ")} Search interest is retained as evidence. Capability and funnel scores must reflect verified Beast/SEANGWORLD fit; trend and YouTube history remain unavailable.</p><div className="mt-3 grid gap-3 sm:grid-cols-3"><Field title="Category" value={draft.category} onChange={(value) => setDraft((current) => ({ ...current, category: value }))} /><NumberField title="Capability match (0–100)" value={draft.capabilityMatch} onChange={(value) => setDraft((current) => ({ ...current, capabilityMatch: Math.min(100, value) }))} /><NumberField title="Funnel value (0–100)" value={draft.funnelValue} onChange={(value) => setDraft((current) => ({ ...current, funnelValue: Math.min(100, value) }))} /></div><button className="beast-button mt-3" disabled={busy || !draft.category.trim()} onClick={() => void onSend("POST", { kind: "evaluate_job", id: job.id, ...draft, audienceInterest, trendOpportunity: null, historicalPerformance: null, evidence: job.topic.evidence || [] }, "Opportunity evaluated against the series threshold; unavailable evidence stayed unavailable.")}>Evaluate opportunity</button></details>;
}
...
function ProductionPlanner({ job, busy, onSend }: { job: Job; busy: boolean; onSend: Send }) {
  ...
  const technicalRetryOnly = job.provenance?.technicalRetryAuthorized === true;
