...
function OwnerWorkflowCard({ job, seriesName, busy, publishingLocked, onSend }: { job: Job; seriesName: string; busy: boolean; publishingLocked: boolean; onSend: Send }) {
  ...
  return <article className="rounded-xl border border-white/10 bg-slate-950/40 p-4">
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      {finished && job.topic.title === "SEANGWORLD News" ? <button className="beast-button" disabled={busy} onClick={() => void onSend("POST", { kind: "prepare_news_visual_test", id: job.id }, "Image-backed test prepared without spending credits. The original video is unchanged.")}>Prepare image-backed test</button> : null}
