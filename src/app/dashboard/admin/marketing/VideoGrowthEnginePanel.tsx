...
export function VideoGrowthEnginePanel() {
  ...
  return <div className="space-y-6">
    <DashboardCard accent="admin">
      <SectionHeader eyebrow="BMKT-005 · Owner only" title="AI Video & YouTube Growth Engine" description="A deterministic control plane for evidence-backed discovery, grounded scripting, provider-neutral production planning, series, presenters, jobs, and future YouTube delivery." />
...
function ProductionPlanner({ job, busy, onSend }: { job: Job; busy: boolean; onSend: Send }) {
  ...
  return <div className="mt-4 rounded-xl border border-amber-300/20 bg-amber-300/[0.04] p-4"><p className="text-sm font-black text-amber-100">Provider-neutral production plan</p>{manifest ? <><p className="mt-2 text-sm text-slate-300">{manifest.scenes.length} scenes · {(manifest.runtimeMs / 1000).toFixed(0)} seconds · {manifest.width}×{manifest.height} · {label(manifest.planState)}</p><p className="mt-1 break-all text-xs text-slate-500">Manifest {manifest.checksum}</p><ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-300">{manifest.blockers.map((item) => <li key={item}>{item}</li>)}</ul></> : <p className="mt-2 text-sm text-slate-300">Builds deterministic scenes, timing, captions, provider slots, retry policy, and provenance requirements without contacting a provider.</p>}<div className="mt-3 flex flex-wrap gap-2"><button className="beast-button" disabled={busy} onClick={() => void onSend("POST", { kind: "plan_production", id: job.id }, manifest ? "Production manifest rebuilt deterministically. No provider was contacted." : "Production manifest prepared. Provider authorization is still required before rendering.")}>{manifest ? "Rebuild manifest" : "Prepare production manifest"}</button><button className="min-h-10 rounded-lg border border-white/10 px-3 text-sm font-black text-slate-500" disabled title="A separately authorized media provider and renderer are required">Generate video · provider gate</button></div></div>;
}
