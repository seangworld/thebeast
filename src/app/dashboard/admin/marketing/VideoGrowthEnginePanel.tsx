...
function OwnerWorkflowCard({ job, seriesName, busy, publishingLocked, onSend }: { job: Job; seriesName: string; busy: boolean; publishingLocked: boolean; onSend: Send }) {
  ...
  return <article className="rounded-xl border border-white/10 bg-slate-950/40 p-4">
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      ...
      {job.revision === 7 && job.provenance?.timingEvidenceRequired === true && job.provenance?.timingEvidenceBound !== true ? <span className="self-center text-xs font-bold text-amber-100">Timing evidence required before render. Bind provider word/phrase timestamps through the controlled timing action.</span> : null}
