...
export function BeastAdminRoadmapIntakeWorkspace() {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          governance_classification: "intake",
          execution_status: "candidate_intake",
          is_next_build: false,
        })
        .select(
          "id,user_id,product_id,title,summary,status,owner_notes,created_at,updated_at"
  ...
  if (loading) {
    return (
      <DashboardCard accent="admin">
        <SectionHeader
          eyebrow="Non-canonical intake"
          title="Loading owner candidates"
          description="BeastAdmin is retrieving candidate ideas and private annotations. These records cannot authorize execution."
        />
    ...
    <div className="space-y-6">
      <DashboardCard accent="admin">
        <SectionHeader
          ...
          title="Candidate intake and private annotations"
          description="These owner-managed rows are planning input only. They cannot approve, authorize, schedule, execute, or override BeastFusion governance."
        />
      </DashboardCard>
      ...
      <DashboardCard accent="admin">
        <SectionHeader
          eyebrow="Candidate portfolio"
          title="Sort ideas before governance review"
          description="Product labels organize intake only. Status choices here are owner annotations and never canonical delivery state."
        />
      ...
      <DashboardCard accent="admin">
        <SectionHeader
          eyebrow="Add Candidate"
          title="Capture an idea for governance review"
          description="Adding a candidate does not approve it or make it executable. BeastFusion must separately record governed work."
        />
        <form onSubmit={addRoadmapItem} className="mt-5 grid gap-4">
          ...
          <div>
            ...
            >
              {adding ? "Adding…" : "Add to candidate intake"}
            </button>
          </div>
        </form>
      </DashboardCard>
        ...
        {visibleItems.length ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-3">
                    ...
                    >
                      {savingId === item.id ? "Saving…" : "Save annotation"}
                    </button>
          ...
          <div className="mt-5 rounded-xl border border-dashed border-[#3b4659] bg-[#111827] p-8 text-center">
            <h3 className="text-xl font-black text-white">
              {items.length
                ? "No candidate items match these filters"
                : "Candidate intake is ready for its first idea"}
            </h3>
