...
export function BeastHunterWorkspace() {
  ...
  async function runOpportunityAction(opportunityId: string, action: "validate" | "build_brief" | "monitor") {
    setUpdatingOpportunityId(opportunityId); setRunError(null);
    try {
      ...
      const body = await response.json() as { error?: string; validation?: BeastHunterRankedCandidate["validation"]; buildBrief?: BeastHunterRankedCandidate["buildBrief"]; trackingStatus?: BeastHunterTrackingStatus; trendStatus?: BeastHunterRankedCandidate["trendStatus"]; lastMonitoredAt?: string };
      if (!response.ok) throw new Error(body.error || "The BeastHunter action could not be completed.");
      setResults((current) => current.map((item) => item.id === opportunityId ? { ...item, ...(body.validation ? { validation: body.validation } : {}), ...(body.buildBrief ? { buildBrief: body.buildBrief } : {}), ...(body.trackingStatus ? { trackingStatus: body.trackingStatus } : {}), ...(body.trendStatus ? { trendStatus: body.trendStatus, lastMonitoredAt: body.lastMonitoredAt } : {}) } : item));
...
function OpportunityCard({ result, expanded, busy, onExpand, onTrack, onAction }: { result: BeastHunterRankedCandidate; expanded: boolean; busy: boolean; onExpand: () => void; onTrack: (status: BeastHunterTrackingStatus) => void; onAction: (action: "validate" | "build_brief" | "monitor") => void }) {
