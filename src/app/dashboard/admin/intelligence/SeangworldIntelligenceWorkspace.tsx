...
} from "@/lib/seangworldAnalyticsScope";
...
function QualifiedTrafficTable({
  ...
  return (
    <section className="rounded-2xl border border-emerald-300/20 bg-[#111827] p-5" aria-labelledby="qualified-traffic-heading">
      ...
      <h2 id="qualified-traffic-heading" className="mt-2 text-xl font-black text-white">Qualified traffic by acquisition campaign and landing page</h2>
      <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-300">
        ...
        {" "}These are intent events, not verified registrations or activations. Each report is limited to 50 rows; missing, ambiguous or suppressed action evidence is unavailable, not zero. Rows join only on the same source, medium, GA4 campaign name/ID and landing page. GA4 labels such as (not set) are not campaign matches; these labels are not automatically linked to BeastMarketing campaigns or proof of causal lift.
      </p>
      {items.length ? (
        <div className="mt-4 overflow-x-auto" tabIndex={0} aria-label="Qualified traffic by acquisition campaign and landing page table, horizontally scrollable">
          <table className="min-w-[76rem] w-full text-left text-sm">
            <thead className="text-slate-400"><tr><th className="p-3">Source</th><th className="p-3">Medium</th><th className="p-3">GA4 campaign</th><th className="p-3">Landing page</th><th className="p-3">Sessions</th><th className="p-3">Change</th><th className="p-3">Engaged</th><th className="p-3">Engagement</th><th className="p-3">Qualified actions</th></tr></thead>
            <tbody>{items.map((item) => <tr key={JSON.stringify([item.source, item.medium, item.campaignName, item.campaignId, item.landingPage])} className="border-t border-white/10 text-slate-200"><td className="p-3 font-bold text-white">{item.source}</td><td className="p-3">{item.medium || "Unavailable"}</td><td className="max-w-xs break-words p-3">{item.campaignName || "Unavailable"}<span className="mt-1 block text-xs text-slate-400">ID: {item.campaignId || "Unavailable"}</span></td><td className="max-w-sm break-words p-3">{item.landingPage}</td><td className="p-3">{number(item.sessions)}</td><td className="p-3">{changeLabel(item.sessionChange)}</td><td className="p-3">{number(item.engagedSessions)}</td><td className="p-3">{item.engagementRate === null ? "Unavailable" : `${(item.engagementRate * 100).toFixed(1)}%`}</td><td className="p-3 font-black text-emerald-200">{item.qualifiedActions === null ? "Unavailable" : number(item.qualifiedActions)}<span className="mt-1 block text-xs font-normal text-slate-500">Prior {item.previousQualifiedActions === null ? "unavailable" : number(item.previousQualifiedActions)}</span></td></tr>)}</tbody>
          </table>
        </div>
...
export function SeangworldIntelligenceWorkspace({
  ...
  const analyticsScope = getSeangworldAnalyticsScope(product);
  useEffect(() => {
    let active = true;
    setLoading(true);
    setError("");
    const params = new URLSearchParams({ days: String(reportingDays) });
    if (product) params.set("product", product);
    void fetch(`/api/admin/seangworld-intelligence?${params}`, { cache: "no-store" })
      .then(async (response) => {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-white/10 bg-[#111827] p-5">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-300">Executive Summary</p><h2 className="mt-2 text-2xl font-black text-white">Verified {analyticsScope ? `${analyticsScope.label} ` : "ecosystem "}signals</h2><p className="mt-2 text-sm text-slate-300">{snapshot.comparisonPeriod}</p>{analyticsScope ? <p className="mt-2 text-xs font-bold text-cyan-200">Filtered by the product&apos;s canonical production hostname.</p> : null}</div><div className="flex flex-wrap items-end gap-3"><label className="grid gap-1 text-xs font-bold text-slate-300">Reporting range<select className="beast-input min-w-32" value={reportingDays} onChange={(event) => setReportingDays(Number(event.target.value))}><option value={7}>7 days</option><option value={30}>30 days</option><option value={90}>90 days</option></select></label><button className="beast-button-secondary" onClick={() => setRefresh((value) => value + 1)}>Refresh status</button></div></div>
