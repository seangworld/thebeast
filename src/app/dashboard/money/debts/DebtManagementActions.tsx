...
export function DebtManagementActions({ debt, fundingSources, history, busy, onPayment, onResetDueDate }: DebtManagementActionsProps) {
  ...
  if (panel === "custom") {
    return <div className="grid min-w-0 gap-3" data-debt-management-workflow="true" role="dialog" aria-label={`${debt.name} custom payment`}>
      ...
      {paymentDetails}
      {actionStatus?.type === "error" ? <p role="alert" className="rounded bg-red-950/60 px-3 py-2 text-sm text-red-200">{actionStatus.message}</p> : null}
      <div className="flex flex-wrap gap-2"><button type="button" disabled={busy || customAmount <= 0} onClick={() => void record("custom", customAmount)} className="beast-button">Confirm Payment</button><button type="button" disabled={busy} onClick={() => { setActionStatus(null); setPanel(null); }} className="beast-button-secondary">Cancel</button></div>
    </div>;
  }
