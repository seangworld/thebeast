...
export function PaymentAutomationControls({ name, autoPayEnabled, reminderEnabled, onSave, compact = false }: {
  ...
  return <div className={`grid min-w-0 gap-2 ${compact ? "grid-cols-2 text-xs" : "text-sm"}`} data-payment-automation-controls="true">
    ...
    <label className={`flex min-h-11 cursor-pointer items-center gap-2 rounded-lg border border-[#2a3242] focus-within:ring-2 focus-within:ring-cyan-300 ${compact ? "px-2" : "px-3"}`}><input type="checkbox" checked={values.reminderEnabled} onChange={(event) => update("reminderEnabled", event.target.checked)} aria-label={`Remind me before ${name} is due`} /><span>Reminder</span></label>
