...
test("migration and responsive controls preserve owner-scoped RLS tables", () => {
  ...
  assert.equal(controls.match(/whitespace-nowrap/g)?.length, 2);
  for (const source of [bills, debts]) {
    ...
    assert.match(source, /className="[^"]*hidden[^"]*min-\[1440px\]:table-cell/);
  ...
  assert.match(debtWorkspace, /width=\{224\}/);
  assert.doesNotMatch(debtWorkspace, /width=\{560\}/);
});
